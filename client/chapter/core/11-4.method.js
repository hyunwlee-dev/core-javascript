...
const shopOrder = {
  ...
  // 화살표 함수는 this를 바인딩하지 않는다. {lime, 3}
  // totalPrice: () => {
  //   console.log(this);
  // }
  totalPrice() {
    //  옛날 방식 {blue, 2}
    // let self = shopOrder;
    // return self.menu.reduce((acc, curr) => acc + curr.price * curr.count, 0);
    // console.log(this.menu[0].price * this.menu[0].count);
    return this.menu.reduce((acc, curr) => acc + curr.price * curr.count, 0);
  }
};
...
navigationMenu.addItem({ id: 'link-y', text: 'Yahoo', link: 'https://yahoo.co.kr' });